...
版本：`context-elasticity-pilot-v1`  
状态：前期实验框架与交互原型  
数据基线日期：2026-08-10
...
当前原型使用 Bitcoin 2017–2024 窗口，共 416 个完整周块，严格复现 Li 项目的“连续 7 个日频 Open 均值”聚合方式。原型中的事件标签只是界面验证材料，不应直接作为正式预注册事件清单。
...
| L4 | USD 价格刻度与点位数值 | 历史事件 |
| L5 | 中性措辞的候选历史事件 | 无 |

主 `trajectory` 组依次经历 L0→L5，保留上一轮边界作为可修改起点。为了分离语义效应和承诺/记忆效应，确认性研究必须同时设置 `snapshot` 组：每名参与者对一条曲线只看到一个披露层级。
...
4. 每层提交边界、信心、理由和耗时；
5. 提交后不可回到上一层；
6. L1–L5 自动继承上一层边界，参与者可以保持不动；
7. 完成后显示个人边界轨迹，但不提示所谓“正确答案”；
8. 服务器按轮次原子保存，页面刷新不应覆盖已经提交的决定；
9. 正式部署前增加退出研究和数据删除机制，并通过所在机构伦理审查。
...
- 提交时对应日期；
- `confidence`、`rationale`、`elapsed_ms`；
