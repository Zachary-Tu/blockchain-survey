...
版本：`context-elasticity-four-asset-v2`  
状态：前期实验框架与交互原型  
数据基线日期：2026-08-11
...
当前 v2 原型使用 Bitcoin、Ethereum、Solana 和 BNB 四条代表性曲线，并统一截取 2020 年 8 月至 2024 年 12 月的共同窗口。每条曲线包含 229–230 个周度点，严格复现 Li 项目的“连续 7 个日频 Open 均值”聚合方式。四条曲线分别在显示窗口内做 min–max 标准化，从而让匿名阶段的纵向可视范围可比；披露真实价格时只更换纵轴和悬停读数，不改变曲线几何形状。原型中的事件标签只是可追溯的界面验证材料，不应直接作为正式预注册事件清单。
...
flowchart LR
    ...
    L1 --> L2["L2 资产身份<br/>名称与代码"]
    L2 --> L3["L3 时间信息<br/>真实日期"]
    L3 --> L4["L4 价格信息<br/>币种与精确数值"]
    L4 --> L5["L5 事件位置<br/>仅编号，不给名称"]
    L5 --> L6["L6 事件语义<br/>日期与中性名称"]
    L6 --> R["边界轨迹与上下文弹性"]
```
...
| L1 | 周序号、标准化纵轴 | 资产、真实时间、货币单位、事件 |
| L2 | 资产名称和代码 | 真实日期、价格、事件 |
| L3 | 真实日期范围和时间刻度 | 价格单位和事件 |
| L4 | USD 价格刻度与点位数值 | 事件位置和名称 |
| L5 | 六个候选事件的编号位置 | 事件名称与内容 |
| L6 | 事件日期和中性名称 | 无 |

主 `trajectory` 组依次经历 L0→L6，保留上一轮边界作为可修改起点。身份先于日期，披露顺序固定而不在被试内随机，避免同一流程产生难以解释的路径组合。L5→L6 的拆分专门用于区分“竖线位置造成的视觉锚定”和“知道事件含义后的语义修正”。为了分离语义效应和承诺/记忆效应，确认性研究仍应设置 `snapshot` 组：每名参与者对一条曲线只看到一个披露层级。
...
3. 四条曲线按会话随机排序；每条曲线内部固定按 L0→L6 展示；
4. L0 首次点击或拖动产生两个边界；
5. 每层提交边界、信心、主观影响、判断依据、理由和耗时；
6. 每次披露先出现独立确认卡；确认后才开始记录本轮作答时间；
7. 提交后不可回到上一层；
8. L1–L6 自动继承上一层边界，参与者可以保持不动；
9. 每完成一条曲线出现休息页，四条曲线共形成 28 次判断；
...
- **H1a**：只显示事件位置的 L5 会产生可测量的视觉锚定效应；
- **H1b**：事件名称披露 L6 相对 L5 的增量修正显著，用以识别超出位置锚定的语义效应；
