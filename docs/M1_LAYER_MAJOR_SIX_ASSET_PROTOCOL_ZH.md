# M1 v4.1：按披露层推进的六资产实验协议

## 1. 本版目的

本版将 M1 从“先完成一个资产的全部披露，再进入下一个资产”改为“先在同一信息状态下依次完成所有资产，再进入下一信息状态”。这样可以降低披露层与资产顺序的系统性混淆，并让每一层形成可直接比较的横截面。
...
七个信息层依次为：G0 → GI1 → GI2 → DI1 → DI2 → DI3 → DI4。每一层内部依次呈现六种资产，因此每位参与者完成 7 × 6 = 42 次独立提交。
...
5. 提交后锁定该判断，进入同层下一资产；六种资产完成后显示层级反馈页。
...
- 人类 M1：`m1-human-main-v2-layer-major-six-assets`
...
数据库唯一键仍由 `session_id + trial_id + disclosure_index` 区分每次回答，无需迁移。会话配置额外保存资产随机顺序、`disclosure-major` 顺序、事件规则和范围控件版本。CSV/JSON 导出继续逐行保存 42 次回答，因此可按披露层、资产、参与者或 Agent 模型直接分组分析。
